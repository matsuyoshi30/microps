...
intr_request_irq(unsigned int irq, int (*handler)(unsigned int irq, void *dev), int flags, const char *name, void *dev)
{
  ...
  for (entry=irqs; entry; entry=entry->next) {
    if (entry->irq == irq) {
      // すでに登録されている IRQ 番号のとき、共有が許可されているかチェック
      // 共有が許可されていれば追加、されていなければエラー
    ...
    default:
      for (entry=irqs; entry; entry=entry->next) {
        if (entry->irq == (unsigned int)sig) {
          ...
          entry->handler(entry->irq, entry->dev); // デバイス割り込み処理
        }
      }
      break;
    }
